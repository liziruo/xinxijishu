...
public class KechengController {
    ...
    @Autowired
    private KechengCollectionService kechengCollectionService;//教学视频收藏
    @Autowired
    private KechengLiuyanService kechengLiuyanService;//教学视频留言
    @Autowired
    private LaoshiService laoshiService;//教师
    @Autowired
                ...
                if(!".xls".equals(suffix)){
                    ...
                    if(!file.exists()){
                        ...
                        for(List<String> data:dataList){
                            //循环
                            KechengEntity kechengEntity = new KechengEntity();
//                            kechengEntity.setLaoshiId(Integer.valueOf(data.get(0)));   //教师 要改的
//                            kechengEntity.setKechengName(data.get(0));                    //教学视频名称 要改的
//                            kechengEntity.setKechengUuidNumber(data.get(0));                    //教学视频编号 要改的
//                            kechengEntity.setKechengPhoto("");//详情和图片
//                            kechengEntity.setKechengFile(data.get(0));                    //教学资料 要改的
//                            kechengEntity.setKechengVideo(data.get(0));                    //教学视频视频 要改的
//                            kechengEntity.setKechengTypes(Integer.valueOf(data.get(0)));   //教学视频类型 要改的
//                            kechengEntity.setKechengClicknum(Integer.valueOf(data.get(0)));   //教学视频热度 要改的
...
//                            kechengEntity.setCreateTime(date);//时间
                        ...
                        if(kechengEntities_kechengUuidNumber.size() >0 ){
                            ...
                            return R.error(511,"数据库的该表中的 [教学视频编号] 字段已经存在 存在数据为:"+repeatFields.toString());
                        }
